...
class DeepNeuralNetwork():
    ...
    def backward_pass(self, y_train, output):
        ...
        # Calculate W2 update
        error = np.dot(params['W3'].T, error) * self.sigmoid(params['Z2'], derivative=True) + 2 * self.lam * np.sum(params["W2"])
        ...
        # Calculate W1 update
        error = np.dot(params['W2'].T, error) * self.sigmoid(params['Z1'], derivative=True) + 2 * self.lam * np.sum(params["W1"])
